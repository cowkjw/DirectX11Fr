...
public:
	...
	// 리소스 등록이 공통된 기능이라 템플릿으로 사용
	template <typename T>
	void RegisterResource(const _wstring& key, T* pResource, unordered_map<_wstring, T*>& resourceMap, vector<_wstring>& keyList)
	{
		auto iter = resourceMap.find(key);
		if (iter == resourceMap.end())
		{
			resourceMap.emplace(key, pResource);
			keyList.push_back(key);
		}
		else
		{
			Safe_Release(iter->second);
			iter->second = pResource;
		}
	}
