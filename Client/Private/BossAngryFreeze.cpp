...
void BossAngryFreeze::Enter(CEnmuMeat* pChar)
{
	...
	leftArm->GetTransform()->Rotate_EulerAngles(_float3(0.0f, 90.f, 0.f)); // 왼팔 회전
	rightArm->GetTransform()->Rotate_EulerAngles(_float3(0.0f, -90.f, 0.f)); // 오른팔 회전
}
...
{
	pChar->GetPart(CEnmuMeat::Parts::LEFTARM)->GetTransform()->Rotate_EulerAngles(_float3(0.0f, -90.f, 0.f)); // 왼팔 원래대로
	pChar->GetPart(CEnmuMeat::Parts::RIGHTARM)->GetTransform()->Rotate_EulerAngles(_float3(0.0f, 90.f, 0.f)); // 오른팔 원래대로
