...
void BossIdle::Update(CEnmuMeat* pChar, _float fTimeDelta)
{
    ...
    if (m_fTimeElapsed < 4.f)
        return;

    _bool allUsed = true;
    for (int i = 0; i < 6; ++i)
        if (!m_bPatternUsed[i]) { allUsed = false; break; }
    if (allUsed)
        for (int i = 0; i < 6; ++i)
    ...
    std::cout << "[BossIdle] Distance to target: " << fDist
    ...
    // 6개 패턴을 순차로 검사
    //    0: BossPunch,   1: BossSwingAttack, 2: BossHandAttack,
    //    3: BossFreeze,  4: BossFollowPunch, 5: BossTentacle
    for (int offset = 0; offset < 6; ++offset)
    {
        ...
        switch (idx)
        {
        case 0: // BossPunch (거리 ≤ 20, 쿨타임 0)
            if (pChar->m_CD_Punch == 0.f && fDist <= 80.f)
            {
                chosenIdx = 0;
            }
            break;

        case 1: // BossSwingAttack (거리  90, 쿨타임 0)
            if (pChar->m_CD_Swing == 0.f && fDist >= 110.f)
            {
                chosenIdx = 1;
            }
            break;

        case 2: // BossHandAttack (거리 ≤ 80, 쿨타임 0)
            if (pChar->m_CD_Hand == 0.f && fDist <= 80.f)
            {
                chosenIdx = 2;
            }
            break;

        case 3: // BossFreezeAttack (거리 ≤ 25, 쿨타임 0)
            if (pChar->m_CD_Freeze == 0.f && fDist <= 100.f)
            {
                chosenIdx = 3;
            }
            break;

        case 4: // BossFollowPunch (거리 ≤ 50, 쿨타임 0)
            if (pChar->m_CD_FollowPunch == 0.f && fDist >= 140.f&& fDist <= 150.f)
            {
                chosenIdx = 4;
            }
            break;

        case 5: // BossTentacle (거리 ≤ 60, 쿨타임 0)
            if (pChar->m_CD_Tentacle == 0.f && fDist >= 160.f)
            {
                chosenIdx = 5;
            }
            break;
